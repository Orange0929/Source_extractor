...
class AudioPlot {
  ...
  draw() {
    ...
    if (!w) return;
    const h = 215, dpr = Math.min(window.devicePixelRatio || 1, 2);
    ...
    for (let t = Math.ceil(a / step) * step; t <= Math.min(end, a + span); t += step) {
      const px = x(t);
      c.strokeStyle = '#263244'; c.beginPath(); c.moveTo(px, 22); c.lineTo(px, 190); c.stroke();
      c.fillStyle = '#bac9dc'; c.fillText(t.toFixed(step < 1 ? 2 : 1) + 's', Math.min(w - 52, Math.max(2, px + 4)), 207);
    }
